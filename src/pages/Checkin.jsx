...
export default function Checkin({ onComplete, editEntry, onClearEdit }) {
  ...
  useEffect(() => {
    if (isEditing) return // skip the "already done" check when editing
    async function check() {
      setLoading(true)
      const existing = await getUserTodayCheckin(session.userId, session.groupId, todayString(), period)
      if (existing) {
        // Pre-fill with existing values so they can edit directly
        setValues({
  ...
  if (submitted) {
    return (
      <div className={styles.submitted + ' animate-fade-up'}>
        ...
        <p>{isEditing ? 'Your entry has been updated.' : `Your ${period} is in the books. ${period === 'morning' ? 'Check back tonight.' : 'See you tomorrow.'}`}</p>
  ...
  return (
    <div className={styles.page}>
      ...
      <Card highlighted style={{ marginBottom: '1.25rem' }}>
        ...
        <textarea className={styles.noteInput} rows={3} placeholder="A thought, a moment, a tiny thing..." value={values.wildcardText} onChange={e => set('wildcardText', e.target.value)} />
      </Card>
