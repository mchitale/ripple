...
export default function Feed({ onEdit }) {
    ...
    <div className={styles.page}>
      ...
      {checkins.map(checkin => (
        <FeedEntry
          key={checkin.id}
          checkin={checkin}
          isMe={checkin.member_id === session.userId}
          onEdit={onEdit}
        />
      ))}
...
function FeedEntry({ checkin, isMe, onEdit }) {
  const { members: member } = checkin

  return (
    <Card style={{ marginBottom: '0.9rem', animation: 'fadeUp 0.4s ease both' }}>
      <div className={styles.entryHeader}>
        ...
        <div className={styles.entryRight}>
          ...
          {isMe && (
            <button className={styles.editBtn} onClick={() => onEdit(checkin)}>Edit</button>
          )}
        </div>
      </div>
